...
import { BaseTank, MediumTank } from './tanks/TankClasses';
...
export class Tank {
    ...
    constructor(scene: THREE.Scene, inputManager: InputManager, onShoot: () => void, tankStats: BaseTank = new MediumTank()) {
        ...
        this.shootDelay = Math.max(0.05, this.tankStats.shootDelay - (upgrades.fireRate * 0.02));

        this.velocity = 0;
        this.maxSpeed = this.tankStats.maxSpeed + (upgrades.speed * 5);
        this.acceleration = this.tankStats.acceleration + (upgrades.speed * 10);
